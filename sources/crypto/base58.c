...
bool b58tobin(void *bin, size_t *binszp, const char *b58, size_t b58sz){
	...
	for ( ; i < b58sz; ++i) {
        if (b58u[i] & 0x80){
            ...
            //printf("High-bit set on invalid digit\n");
			return false;
        }
        if (b58digits_map[b58u[i]] == -1){
			// Invalid base58 digit
            //printf("Invalid base58 digit = %d\n",i);
			return false;
        }
...
//            printf("Output number too big (carry to the next int32)\n");
			return false;
        }
        if (outi[0] & zeromask){
			// Output number too big (last int32 filled too far)
//            printf("Output number too big (last int32 filled too far)\n");
			return false;
        }
	}
